...
const baseUrl$5 = "https://d000d.com";
...
const baseUrl$4 = "https://lmscript.xyz";
...
const baseUrl$3 = "https://tugaflix.best/";
...
const hlsProxyUrl = "https://m3u8proxy.techygiraffe.workers.dev/";
...
async function getVideowlUrlStream(ctx, decryptedId) {
  var _a;
  const sharePage = await ctx.fetcher(hlsProxyUrl, {
    query: {
      url: encodeURIComponent("https://cloud.mail.ru/public/uaRH/2PYWcJRpH")
    }
  });
      ...
      stream: [
        {
          ...
          flags: [flags.CORS_ALLOWED],
          captions: [],
          playlist: `${hlsProxyUrl}?url=${encodeURIComponent(streamUrl)}`
        }
      ]
    };
  }
...
const baseUrl$2 = "https://www.goojara.to";
...
function parseVideoLinks(inputString) {
  ...
  linksArray.forEach((link) => {
    ...
    if (match) {
      ...
      result[validQuality] = {
        type: "mp4",
        url: `https://mw-simpleproxy.techygiraffe.workers.dev/?destination=${encodeURIComponent(mp4Url)},`
      };
    }
  });
  return result;
...
const universalScraper$1 = async (ctx) => {
  const searchResult = await ctx.fetcher(hlsProxyUrl, {
    query: {
